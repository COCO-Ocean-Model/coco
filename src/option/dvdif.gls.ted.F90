...
!--- TED
  ...
  real(8)       ::    gint(nxydim)
  real(8)       ::  tedn3d(nxydim, nzdim)
  real(8)       ::  tedf3d(nxydim, nzdim)
  real(8)       ::  dzmsig(nxydim, nzdim)
  real(8)       ::   ted3d(nxydim, nzdim)
...
!--- TED
     ...
     write(jfpar, nmdved)
     if ( iam == 0 ) then
        write(jfpar, *) ' Turbulent energy dissipation rate is not used.'
        do ij = 1, nxydim
           tedn2d(ij) = 0.d0
           tedf2d(ij) = 0.d0
        end do
...
! far-field
  ...
  call chekin(tedf3d,   'TEDF', &
       &               'far ted calculated in model', 'cm^2/s^3', &
       &          nx,       ny,       nz,     nxyzdm, 'OCLVMT')
!---
