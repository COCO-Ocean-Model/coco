...
!=======================================================================
#ifndef OPT_TRIPOLE
  subroutine shift1(q1,  idim, jdim, kdim )
#else
  subroutine shift1(q1,  idim, jdim, kdim, fact,  ioff, joff )
#endif
    implicit none
...
!=======================================================================
#ifndef OPT_TRIPOLE
  subroutine shift2(q1, q2, idim, jdim, kdim )
#else
  subroutine shift2(q1, q2, idim, jdim, kdim, fact,  ioff, joff )
#endif
    implicit none
    real(8),                  intent(inout)  ::    q1(1:idim,1:jdim,1:kdim)
...
!=======================================================================
#ifndef OPT_TRIPOLE
  subroutine shift3(q1, q2, q3, idim, jdim, kdim )
#else
  subroutine shift3(q1, q2, q3, idim, jdim, kdim, fact,  ioff, joff )
#endif
    implicit none     
    real(8),                  intent(inout)  ::    q1(1:idim,1:jdim,1:kdim)
    real(8),                  intent(inout)  ::    q2(1:idim,1:jdim,1:kdim)
...
!       end do
       do k = 1, ksize
          do j = 0, jcomm
             do i = istr+ioff2, iend+ioff2
                tri_send(i,j,k0+k) = fact2 * qq(nxdim+1-i+ioff2, jend+1-j+joff2, k)
             end do
          end do
       end do
    endif
...
!=======================================================================
  subroutine shift_pack_end
    ...
    call shiftx(west_recv, east_recv, west_send, east_send, nelems)
    ...
    call shifty(south_recv, north_recv, south_send, north_send, nelems)
    ...
    if (is_tri_edge) then
       ...
       call shift_tri_edge(tri_recv, tri_send, nelems )
    endif
#endif
    pack_mode = .false.
...
#include "mpif.h"
    ...
    if (is_tri_edge) then
       ...
       if (ioffs(id) .eq. -1) then
          ...
          call shiftx(tri_west_recv, tri_east_recv, tri_west_send, tri_east_send, nelems )
...
#include "mpif.h"
    ...
    call mpi_sendrecv(sendbuf, nelems, mpi_double_precision, idest, itag, &
         &            recvbuf, nelems, mpi_double_precision, isrc,  itag, &
...
!=======================================================================
  ...
  subroutine shiftx(rvbfx1l, rvbfx2l, sdbfx1l, sdbfx2l, nbfdim)
    implicit none
#include "mpif.h"
    real(8),      intent(inout) ::  rvbfx1l(:,:,:), rvbfx2l(:,:,:)
    real(8),      intent(in)    ::  sdbfx1l(:,:,:), sdbfx2l(:,:,:)
    integer(4),   intent(inout) ::  nbfdim

!---- internal work
    integer(4)  ::  isrqx1, isrqx2, irrqx1, irrqx2
    integer(4)  ::  istmpi(mpi_status_size)

    call mpi_isend(sdbfx1l, nbfdim, mpi_real8, idown, 1, mpi_comm_ogcm, isrqx1, ierr)
    call mpi_isend(sdbfx2l, nbfdim, mpi_real8, iup,   2, mpi_comm_ogcm, isrqx2, ierr) 
    call mpi_irecv(rvbfx2l, nbfdim, mpi_real8, iup,   1, mpi_comm_ogcm, irrqx1, ierr)
    call mpi_irecv(rvbfx1l, nbfdim, mpi_real8, idown, 2, mpi_comm_ogcm, irrqx2, ierr)
...
! *********************************************************************
  ...
  subroutine shifty(rvbfy1l, rvbfy2l, sdbfy1l, sdbfy2l, nbfdim)
    implicit none
#include "mpif.h"
    real(8),      intent(inout) ::  rvbfy1l(:,:,:), rvbfy2l(:,:,:)
    real(8),      intent(in)    ::  sdbfy1l(:,:,:), sdbfy2l(:,:,:)
    integer(4),   intent(inout) ::  nbfdim
!---- internal work
    integer(4)  ::  isrqy1, isrqy2, irrqy1, irrqy2
    integer(4)  ::  istmpi(mpi_status_size)

    call mpi_isend(sdbfy1l, nbfdim, mpi_real8, jdown, 3, mpi_comm_ogcm, isrqy1, ierr)
    call mpi_isend(sdbfy2l, nbfdim, mpi_real8, jup,   4, mpi_comm_ogcm, isrqy2, ierr)
    call mpi_irecv(rvbfy2l, nbfdim, mpi_real8, jup,   3, mpi_comm_ogcm, irrqy1, ierr)
    call mpi_irecv(rvbfy1l, nbfdim, mpi_real8, jdown, 4, mpi_comm_ogcm, irrqy2, ierr)
