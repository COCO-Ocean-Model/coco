...
!
!  HISTORY
!     '25.04.07  M.Kurogi: from bshft.F90
! ---------------------------------------------------------------------

#ifdef OPT_WIDE_BT_SHFT
  use zocdim, only : nx, ny, nzdim, ntdim, ierr, myrank, jrank, inodes, jnodes, &
      & iup, idown, jup, jdown, jupw, jupe, oinit, mpi_comm_ogcm
#else
  use zocdim
#endif
...
!  [internal save]
  ...
  integer, parameter :: nydim = ny + 2*jcomm
  integer, parameter :: iend = istr + nx -1
  integer, parameter :: jend = jstr + ny -1

  integer, parameter :: istr_w=istr, jstr_w=jstr
  integer, parameter :: iend_w=iend, jend_w=jend
  integer, parameter :: nxdim_w=nxdim, nydim_w=nydim, nxydim_w=nxdim_w*nydim_w
#endif
  ...
  public  ::  bt_shift1,  bt_shift2,  bt_shift3, bt_shift_pack_begin, bt_shift_pack_end, bt_shift_unpack
#ifdef OPT_WIDE_BT_SHFT
  public  ::  istr_w, jstr_w, iend_w, jend_w, nxdim_w, nydim_w, nxydim_w, ncomm
#endif
contains

!=======================================================================
  subroutine bt_shift_pack_begin
    implicit none
...
#include "mpif.h"
  ...
  end subroutine bt_shift_pack_begin

!=======================================================================
  subroutine bt_shift_pack(qq, ksize, fact2, ioff2, joff2)
    real(8) :: qq(:,:,:)
    integer :: ksize
    real(8) :: fact2
    integer :: ioff2, joff2
...
#endif
    !$acc wait
  end subroutine bt_shift_pack

!=======================================================================
  subroutine bt_shift_pack_end
    integer :: nelems, koff
...
#endif
    pack_mode = .false.
  end subroutine bt_shift_pack_end

!=======================================================================
  subroutine bt_shift_unpack(q1, id)
    implicit none
#include "mpif.h"
...
#endif
  end subroutine bt_shift_unpack
...
!=======================================================================
  ...
  subroutine bt_shift1(                                               &
    ...
    if (pack_mode) then
       call bt_shift_pack(q1, kdim, fact, ioff, joff)
    else
       call bt_shift_pack_begin
       call bt_shift_pack(q1, kdim, fact, ioff, joff)
       call bt_shift_pack_end
       call bt_shift_unpack(q1, 1)
    end if
    shift_gpu=.true.
  end subroutine bt_shift1


  subroutine bt_shift2(                                               &
    &                 q1,     q2,                                     &
    &               idim,   jdim,   kdim,                             &
    &              fact0,  ioff0,   joff0)
    implicit none
    ...
    if (pack_mode) then
       call bt_shift_pack(q1, kdim, fact, ioff, joff)
       call bt_shift_pack(q2, kdim, fact, ioff, joff)
    else
       call bt_shift_pack_begin
       call bt_shift_pack(q1, kdim, fact, ioff, joff)
       call bt_shift_pack(q2, kdim, fact, ioff, joff)
       call bt_shift_pack_end
       call bt_shift_unpack(q1, 1)
       call bt_shift_unpack(q2, 2)
    end if
    shift_gpu=.true.
  end subroutine bt_shift2

  subroutine bt_shift3(                   &
    &                 q1,     q2,     q3, &
    &               idim,   jdim,   kdim, &
    &              fact0,  ioff0,   joff0)
    ...
    if (pack_mode) then
       call bt_shift_pack(q1, kdim, fact, ioff, joff)
       call bt_shift_pack(q2, kdim, fact, ioff, joff)
       call bt_shift_pack(q3, kdim, fact, ioff, joff)
    else
       call bt_shift_pack_begin
       call bt_shift_pack(q1, kdim, fact, ioff, joff)
       call bt_shift_pack(q2, kdim, fact, ioff, joff)
       ...
       call bt_shift_unpack(q1, 1)
       call bt_shift_unpack(q2, 2)
       call bt_shift_unpack(q3, 3)
    end if
    shift_gpu=.true.
  end subroutine bt_shift3
