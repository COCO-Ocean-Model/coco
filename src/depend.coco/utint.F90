...
!---- local variables
    integer(4), parameter      ::  nitem = ntdim * 2 + 2
    ...
    character(len=ncf)  ::  cftaux,   cftauy
    character(len=ncf)  ::  cftsfc(1:ntdim)
    character(len=ncf)  ::  cftdmp(1:ntdim)
    namelist /nmsfbc/ cftaux, cftauy, cftsfc, cftdmp
    data cftaux, cftauy / 'not-specified', 'not-specified' /
    data cftsfc / ntdim*'not-specified' /
    ...
    if (iitem == 1) then
       ...
       cfitem = cftauy
    else if (iitem <= ntdim+2  ) then
       cfitem = cftsfc(iitem-2)
    else if (iitem <= ntdim*2+2) then
       cfitem = cftdmp(iitem-ntdim-2)
    else
